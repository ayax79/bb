 ...
 * @version $Revision: $
 */
public class SaveSenderMediaToActivityStream {
    final private static Logger logger = LoggerFactory.getLogger(SaveSenderMediaToActivityStream.class);
        ...
        if (camelMessage != null) {
            try {
                ...
                if (isStreamMedia(metaData)) {
                    activityStreamDao.saveSender(metaData, metaData.getSender());
                }
